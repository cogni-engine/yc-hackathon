 ...
 * Used by /api/ai to ground answers in the company brain, and by the /gbrain
 * inspector page (which reads the in-memory query log below).
 */
...
// In-memory ring buffer of recent queries. Per-instance (fine for the inspector;
// on serverless it won't be shared across instances — a debug tool, not state).
const LOG_MAX = 100;
const log: GbrainLogEntry[] = [];
let seq = 0;

export function recordQuery(entry: Omit<GbrainLogEntry, 'id' | 'at'>): GbrainLogEntry {
  const e: GbrainLogEntry = { ...entry, id: String(++seq), at: new Date().toISOString() };
  log.unshift(e);
...
/** Query the brain and record it in the log (source distinguishes AI vs manual). */
